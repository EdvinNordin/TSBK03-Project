...
public class Fluid
{
    ...
    public float renderD(int i)
    {
                //square (i,j) color from this.density[i];
...
void Start()
{
    ...
    GameObject[] points = new GameObject[N * N];
    int temp = 0;
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            temp = IX(i, j, N);
            points[temp] = Instantiate(prefab, new Vector3(i, j, 0), Quaternion.identity);
        }
    }
...
void addDensity(int x, int y, float amount, int N)
{
    int index = IX(x, y, N);
    this.density[index] += amount;
}
